...
FID Helm chart provides a robust logging system to collect logs from different FID application log files and forward them to multiple destinations like Elasticsearch, OpenSearch, Splunk, and Sumologic.

## Enabling Logging

To enable logging, both the metrics and fluentd configurations need to be set to "true":
...
By default, all major logs are collected. Each log can be individually enabled or disabled through the values.yaml file:
...
- **enabled**: Set to true/false to enable/disable collection of this log
- **path**: Path to the log file
- **index**: Default index name used for the log (can be overridden by custom_index)
- **custom_index**: (Optional) Custom index name to override the default
- **aggregators**: (Optional) List of named aggregators to use for this log

## Aggregator Configuration

Logs can be sent to multiple destinations by configuring aggregators. There are two types of aggregators:

1. **Named Aggregators**: Only receive logs explicitly assigned to them
2. **Unnamed Aggregators**: Receive all enabled logs by default

### Aggregator Example

```yaml
aggregators:
  # Named aggregator - only receives logs that explicitly list "sumologic"
  - name: "sumologic"
    type: "sumologic"
    endpoint: "https://endpoint.sumologic.com/receiver/v1/http/xxx"

  # Unnamed aggregator - receives ALL logs by default
  - type: "elasticsearch"
    host: "elasticsearch-master.svc.cluster.local"
    port: "9200"

  # Named aggregator - only receives logs that explicitly list "splunk"
  - name: "splunk"
    type: "splunk_hec"
    hec_host: "splunk-standalone.svc.cluster.local"
    hec_port: "8088"
...
To send a log to a specific named aggregator, use the `aggregators` field:

```yaml
logs:
  # This log goes to sumologic only
  vds_server:
    enabled: true
    path: "/opt/radiantone/vds/vds_server/logs/vds_server.log"
    index: vds_server.log
    aggregators: [sumologic]

  # This log goes to splunk only
  vds_events:
    enabled: true
    path: "/opt/radiantone/vds/vds_server/logs/vds_events.log"
    index: vds_events.log
  ...
  # This log goes to both splunk and sumologic
  alerts:
    enabled: true
    path: "/opt/radiantone/vds/vds_server/logs/alerts.log"
    index: alerts.log
    aggregators: [splunk, sumologic]

  # This log goes to all aggregators (since no "aggregators" field is specified)
  # It goes to ALL unnamed aggregators and NONE of the named aggregators
  periodiccache:
    enabled: true
...
- Logs without an `aggregators` field go to all unnamed aggregators
- Logs with an empty list `aggregators: []` don't go to any aggregators
- Logs with a list of names `aggregators: [name1, name2]` only go to those specific named aggregators
- Unnamed aggregators always get all logs unless specifically excluded

## Supported Aggregator Types

Different types of destinations require different configurations:

- **elasticsearch**: Elasticsearch destination
- **opensearch**: OpenSearch destination
- **splunk_hec**: Splunk HTTP Event Collector
- **sumologic**: Sumo Logic HTTP collector
...
- For steps to deploy Kibana, please refer to the documentation [here](https://artifacthub.io/packages/helm/elastic/kibana)
